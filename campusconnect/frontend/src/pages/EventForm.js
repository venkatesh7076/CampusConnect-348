  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const eventData = {
        ...
        location:
          formData.location ||
          `${formData.venue.building}, ${formData.venue.room}`, // Default to building+room if not specified
        capacity: formData.capacity || 10, // Default capacity
        registrationDeadline:
          formData.registrationDeadline ||
          new Date(
            new Date(formData.startDate).getTime() - 24 * 60 * 60 * 1000
          ).toISOString(), // Default to 1 day before event
        status: "upcoming",
      };
      ...
      if (response && response.status >= 200 && response.status < 300) {
        ...
        console.log(
          "Event saved successfully, forcing page reload to show new event"
        );
        window.location.href = "/";
      } else {
        setError(
          `Unexpected response: ${response ? response.status : "No response"}`
        );
      }
    } catch (err) {
      ...
      if (err.response) {
        ...
        setError(
          "No response received from the server. Please check your backend connection."
        );
      }
    } finally {
