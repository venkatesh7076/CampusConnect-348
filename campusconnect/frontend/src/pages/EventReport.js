...
const EventReport = () => {
  ...
  const handleGenerateReport = () => {
    ...
    if (endDate) {
      filteredEvents = filteredEvents.filter(
        (event) => new Date(event.endDate) <= new Date(endDate + "T23:59:59")
      );
    }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="text-center mb-10">
        ...
        <p className="text-gray-600 max-w-3xl mx-auto">
          Generate reports on events based on date range, club, and venue to
          track activity across campus.
        </p>
      </div>
              ...
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  ...
                  <div className="bg-indigo-50 p-4 rounded-lg">
                    ...
                    <p className="text-3xl font-bold text-indigo-700">
                      {Object.entries(reportData.stats.eventsByCategory)
                        .length > 0
                        ? Object.entries(
                            reportData.stats.eventsByCategory
                      ...
                      {reportData.events.map((event) => (
                        <tr key={event._id}>
                          ...
                          <td className="px-3 py-4 text-sm text-gray-500">
                            ...
                            event.clubId.name
                              ? event.clubId.name
                              : clubs.find((c) => c._id === event.clubId)
                                  ?.name || "Unknown Club"}
                          </td>
                          <td className="px-3 py-4 text-sm text-gray-500">
                            {event.category}
