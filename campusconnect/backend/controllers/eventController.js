...
exports.getEventById = async (req, res) => {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(eventId) && !isNaN(eventId)) {
      console.log(
        `Invalid ObjectId format: ${eventId} - Attempting to handle numeric ID`
      );
    ...
    try {
      ...
      if (
        typeof EventOrganizer !== "undefined" &&
        EventOrganizer.getEventOrganizers
      ) {
...
exports.createEvent = async (req, res) => {
  try {
    ...
    if (deadlineObj >= startDateObj) {
      return res
        .status(400)
        .json({ message: "Registration deadline must be before start date" });
    }
...
exports.updateEvent = async (req, res) => {
  try {
    ...
    const userId = req.user ? req.user.id : "67e988e5d2130196d11add79"; // Default ID
    const isOrganizer = await EventOrganizer.isEventOrganizer(
      event._id,
      userId
    );
    ...
    if (event.status === "completed" || event.status === "cancelled") {
      return res
        .status(400)
        .json({ message: `Cannot update a ${event.status} event` });
    }
...
exports.deleteEvent = async (req, res) => {
  try {
    ...
    if (event.status === "completed") {
      return res
        .status(400)
        .json({ message: "Cannot delete a completed event" });
    }
  ...
  try {
    ...
    if (!isOrganizer && req.user.role !== "admin") {
      return res
        .status(403)
        .json({
          message: "Not authorized to manage organizers for this event",
        });
    }
    ...
    const updatedOrganizers = await EventOrganizer.find({
      eventId: id,
